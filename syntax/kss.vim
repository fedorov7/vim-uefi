" Vim syntax file
" Language:     KSS
" Maintainer:   Alexander Fedorov <fedorov7@gmai.com>
" Last Change:  2014 Nov 25
"
...
syn keyword uefiSuccess     EFI_SUCCESS TRUE ESUCCESS
syn keyword uefiError       ASSERT ASSERT_EFI_ERROR FALSE EFI_ERROR EFI_LOAD_ERROR EFI_INVALID_PARAMETER EFI_UNSUPPORTED EFI_BAD_BUFFER_SIZE EFI_BUFFER_TOO_SMALL EFI_NOT_READY EFI_DEVICE_ERROR EFI_WRITE_PROTECTED EFI_OUT_OF_RESOURCES EFI_VOLUME_CORRUPTED EFI_VOLUME_FULL EFI_NO_MEDIA EFI_MEDIA_CHANGED EFI_NOT_FOUND EFI_ACCESS_DENIED EFI_NO_RESPONSE EFI_NO_MAPPING EFI_TIMEOUT EFI_NOT_STARTED EFI_ALREADY_STARTED EFI_ABORTED EFI_ICMP_ERROR EFI_TFTP_ERROR EFI_PROTOCOL_ERROR EFI_INCOMPATIBLE_VERSION EFI_SECURITY_VIOLATION EFI_CRC_ERROR EFI_END_OF_MEDIA EFI_END_OF_FILE EFI_INVALID_LANGUAGE EFI_COMPROMISED_DATA EFI_WARN_UNKNOWN_GLYPH EFI_WARN_DELETE_FAILURE EFI_WARN_WRITE_FAILURE EFI_WARN_BUFFER_TOO_SMALL EFI_WARN_STALE_DATA
syn keyword kssMacroMem     SAFE_FREE FREE_MEM_POINTER FREE_PAGES FREE_JSON_OBJECT JSON_OBJECT_FREE FREE_MEM_PAGES EFI_SIZE_TO_PAGES SET_VALUE WAITING WAIT_FOR_KEY
syn keyword kssMacroDebug   DEBUG DBG_PRINT DBG_ERROR DBG_ERROR1 DBG_INFO DBG_INFO1 DBG_WARN DBG_WARN1 DBG_VERBOSE DBG_VERBOSE1 DBG_CURRENT DBG_CURRENT1
syn keyword kssMacroExit    DBG_ENTER DBG_EXIT_STATUS DBG_EXIT_STRING DBG_EXIT DBG_EXIT_DEC DBG_EXIT_HEX DBG_EXIT_TF DBG_EXIT_ERROR DBG_EXIT_ASCII_STRING DBG_EXIT_POINTER
syn keyword kssMacroHelper  FIRST FIRST_HELPER REST REST_HELPER REST_HELPER2 REST_HELPER_ONE REST_HELPER_TWOORMORE COUNT_OF_ARGUMENTS SELECT_40TH DBG_CURRENT_ONE DBG_CURRENT_TWOORMORE DBG_CURRENT_REST DBG_CURRENT_VA DBG_CURRENT_VA2 DBG_CURRENT_STATUS DBG_CURRENT_STATUS_ONE DBG_CURRENT_STATUS_TWOORMORE DBG_CURRENT_STATUS_REST DBG_CURRENT_STATUS_VA DBG_CURRENT_STATUS_VA2
syn keyword kssMacroReturn  RETURN_IF_EFI_ERROR RETURN_IF_EFI_SUCCESS RETURN_IF_NULL RETURN_IF RETURN_EFI_STATUS RETURN_EFI_STRING RETURN_ASCII_STRING RETURN_HEX RETURN_INT RETURN_BOOLEAN RETURN_IF_NULL_EFI_STRING RETURN_BOOLEAN_IF RETURN_BOOLEAN_IF_NULL RETURN_POINTER RETURN_NULL_IF_NULL RETURN_POINTER_IF_NULL RETURN_POINTER_IF RETURN_NUMBER RETURN_NUMBER_IF RETURN_NUMBER_IF_NULL RETURN_NUMBER_IF_EFI_ERROR RETURN_POINTER_IF_EFI_ERROR RETURN_VOID_IF_EFI_ERROR RETURN_VOID_IF_EFI_SUCCESS RETURN_VOID_IF_NULL RETURN_VOID_IF RETURN_VOID_EFI_STATUS RETURN_NULL_IF_EFI_ERROR RETURN_EFI_STRING_IF RETURN_HEX_IF
syn keyword kssMacroGoto    GOTO_IF_EFI_ERROR GOTO_IF_NULL GOTO_IF GOTO
syn keyword kssMacroPrint   DBG_CHAR DBG_HEX DBG_STRING DBG_UNICODE_STRING DBG_CHAR_BUFFER DBG_BUFFER DBG_DEVICE_PATH DBG_JSON_HANDLE DBG_EFI_TIME DBG_MAC_ADDR DBG_IP_ADDR
syn keyword kssMacroBreak   BREAK_IF BREAK_IF_EFI_ERROR BREAK_IF_NULL
syn keyword kssMacroContinue CONTINUE_IF CONTINUE_IF_EFI_ERROR CONTINUE_IF_NULL

syn keyword uefiType        EFIAPI IN OUT BOOLEAN INT8 UINT8 INT16 UINT16 INT32 UINT32 INT64 UINT64 UINTN INTN CHAR8 CHAR16 VOID LIST_ENTRY EFI_GUID EFI_STATUS EFI_HANDLE EFI_EVENT EFI_TPL EFI_LBA EFI_PHYSICAL_ADDRESS EFI_VIRTUAL_ADDRESS EFI_HII_HANDLE BYTE EFI_INPUT_KEY

syn keyword errnoValue      errno EFIerrno ERESTART EMINERRORVAL EPERM ENOENT ESRCH EINTR EIO ENXIO E2BIG ENOEXEC EBADF ECHILD EDEADLK ENOMEM EACCES EFAULT ENOTBLK EBUSY EEXIST EXDEV ENODEV ENOTDIR EISDIR EINVAL ENFILE EMFILE ENOTTY ETXTBSY EFBIG ENOSPC ESPIPE EROFS EMLINK EPIPE EDOM ERANGE EAGAIN EWOULDBLOCK EINPROGRESS EALREADY ENOTSOCK EDESTADDRREQ EMSGSIZE EPROTOTYPE ENOPROTOOPT EPROTONOSUPPORT ESOCKTNOSUPPORT EOPNOTSUPP ENOTSUP EPFNOSUPPORT EAFNOSUPPORT EADDRINUSE EADDRNOTAVAIL ENETDOWN ENETUNREACH ENETRESET ECONNABORTED ECONNRESET ENOBUFS EISCONN ENOTCONN ESHUTDOWN ETOOMANYREFS ETIMEDOUT ECONNREFUSED ELOOP ENAMETOOLONG EHOSTDOWN EHOSTUNREACH ENOTEMPTY EPROCLIM EUSERS EDQUOT ESTALE EREMOTE EBADRPC ERPCMISMATCH EPROGUNAVAIL EPROGMISMATCH EPROCUNAVAIL ENOLCK ENOSYS EFTYPE EAUTH ENEEDAUTH EIDRM ENOMSG EOVERFLOW EILSEQ ENOTHING_1 ECANCELED EBADMSG ENODATA ENOSR ENOSTR ETIME ENOATTR EDOOFUS EMULTIHOP ENOLINK EPROTO EBUFSIZE
...
hi def link uefiMacros        x039_DeepSkyBlue1
...
hi          kssMacroExit      ctermfg=Magenta
...
hi def link kssMacroContinue  x078_SeaGreen3
